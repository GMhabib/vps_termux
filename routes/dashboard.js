...
router.get('/user/get-content/:filename', isAuthenticated, (req, res) => {
    ...
    const stats = fs.statSync(filePath);
    if (stats.size > 10 * 1024 * 1024) { // Limit 10MB
...
router.post('/admin/execute-command', isAuthenticated, isAdmin, (req, res) => {
    ...
    const dangerousCommands = [
        ...
        /\b(useradd|usermod|userdel|groupadd|groupmod|groupdel|passwd|etc\/passwd|etc\/shadow)\b/i,
        /\b(mv|cp)\s+.*\s+\/(\s*|$)/i 
    ];

    if (dangerousCommands.some(regex => regex.test(command))) {
        return res.status(403).json({ output: 'Perintah sistem yang berpotensi merusak dilarang.' });
    }
